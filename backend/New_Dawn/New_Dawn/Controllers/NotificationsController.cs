...
public class NotificationsController(AppDbContext db, UserManager<ApplicationUser> userManager, NotificationService notificationService) : ControllerBase
{
        ...
        [FromQuery] bool? unreadOnly = null)
    {
        ...
        // User sees notifications targeted to them by UserId,
        // OR broadcast notifications matching their role (UserId == null && TargetRole matches)
    ...
    public async Task<IActionResult> Snooze(int id, [FromQuery] int months = 1)
    {
        ...
        // Record when to remind again by creating a group key for the snooze expiry
        var user = await userManager.GetUserAsync(User);
        if (user == null)
            return Unauthorized();
        ...
        // Store snooze as a read notification with the snooze key so the generator can check it
