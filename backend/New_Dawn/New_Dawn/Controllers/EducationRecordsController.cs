...
public class EducationRecordsController(AppDbContext db) : ControllerBase
{
    ...
    {
        var entity = await db.EducationRecords.AsNoTracking().FirstOrDefaultAsync(e => e.EducationRecordId == id);
        if (entity == null) return NotFound(new { success = false, message = "Not found" });
        return Ok(entity);
    }

    [HttpPost]
    public async Task<IActionResult> Create([FromBody] EducationRecord entity)
    {
        db.EducationRecords.Add(entity);
        await db.SaveChangesAsync();
        return CreatedAtAction(nameof(GetById), new { id = entity.EducationRecordId }, entity);
    }

    [HttpPut("{id}")]
    public async Task<IActionResult> Update(int id, [FromBody] EducationRecord entity)
    {
        if (id != entity.EducationRecordId) return BadRequest(new { success = false, message = "ID mismatch" });
        var existing = await db.EducationRecords.FindAsync(id);
    ...
    public async Task<IActionResult> Delete(int id, [FromQuery] bool confirm = false)
    {
        ...
        var entity = await db.EducationRecords.FindAsync(id);
        if (entity == null) return NotFound(new { success = false, message = "Not found" });
