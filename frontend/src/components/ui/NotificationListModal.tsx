    ...
    <table className="w-full text-sm">
      <thead>
        <tr className="border-b-2 border-slate-navy/20 dark:border-white/20">
          ...
          <th className="pb-2 pr-4 text-left font-semibold text-slate-navy dark:text-white">Phone Number</th>
          <th className="pb-2 pr-4 text-left font-semibold text-slate-navy dark:text-white">Email</th>
      ...
      <tbody>
        {items.map((item, index) => (
          <tr key={item.supporterId || index} className="border-b border-slate-navy/5 dark:border-white/5">
            <td className="py-2.5 pr-4 text-slate-navy dark:text-white">{item.displayName}</td>
            <td className="py-2.5 pr-4 text-slate-navy/70 dark:text-white/70">{item.phoneNumber || '--'}</td>
            <td className="py-2.5 pr-4 text-slate-navy/70 dark:text-white/70">{item.email || '--'}</td>
            <td className="py-2.5 pr-4">
              <span className="rounded-full bg-coral-pink/20 px-2 py-0.5 text-xs font-semibold text-slate-navy dark:bg-coral-pink/25 dark:text-white">
...
function ResidentTable({ items }: { items: ResidentListItem[] }) {
  return (
    <table className="w-full text-sm">
      <thead>
        <tr className="border-b-2 border-slate-navy/20 dark:border-white/20">
          <th className="pb-2 pr-4 text-left font-semibold text-slate-navy dark:text-white">Internal Code</th>
          <th className="pb-2 pr-4 text-left font-semibold text-slate-navy dark:text-white">Case Control No</th>
          <th className="pb-2 pr-4 text-left font-semibold text-slate-navy dark:text-white">Safe House</th>
          <th className="pb-2 pr-4 text-left font-semibold text-slate-navy dark:text-white">Risk Level</th>
        </tr>
      </thead>
      <tbody>
        {items.map((item, index) => (
          <tr key={item.residentId || index} className="border-b border-slate-navy/5 dark:border-white/5">
            <td className="py-2.5 pr-4 font-medium text-slate-navy dark:text-white">{item.internalCode}</td>
            <td className="py-2.5 pr-4 text-slate-navy/70 dark:text-white/70">{item.caseControlNo}</td>
            <td className="py-2.5 pr-4 text-slate-navy/70 dark:text-white/70">{item.safehouse}</td>
            <td className="py-2.5 pr-4">
