...
export function MfaSnoozeModal({ isOpen, onClose, onSnooze, isLoading }: MfaSnoozeModalProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/50 px-4 py-8 sm:items-center sm:p-6" onClick={onClose}>
      <div
        className="my-auto w-full max-w-sm rounded-xl border border-slate-navy/10 bg-white p-6 shadow-xl dark:border-white/10 dark:bg-dark-surface"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Clock size={18} className="text-golden-honey" />
            <h3 className="text-lg font-semibold text-slate-navy dark:text-white">Snooze MFA Reminder</h3>
          </div>
          <button
            onClick={onClose}
